...
## Premium look is built in (don't hand-roll styling)
The templates ship a polished, professional default design that adapts to light/dark automatically — a subtly tinted background, elevated cards with soft shadows and hover lift, gradient badges/buttons, gradient headlines and KPI numbers (with a safe solid fallback), refined tables and tabs. **Just use the provided classes** (`.card`, `.grid`, `.kpi` + `.n`/`.l`, `.badge`, `.btn`, `.tip`, tables) and the design comes through. Extra CSS-var tokens are available if you need them: `--accent2` (gradient partner), `--elev` (raised surface), `--shadow`, `--ring`, `--muted`. Don't add your own colors/backgrounds that bypass the variables (it breaks theme-switching). **Never fade a gradient to `transparent`** — it interpolates through black and greys out light mode; fade to a same-hue `rgba(…,0)` instead (the templates already do).
